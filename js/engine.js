...
function titleScreen(){
  ...
  if(!P.runs){
    el.innerHTML='<span class="muse">You are on the riverbank, in the warm afternoon, growing sleepy over your sister\'s book — the one with no pictures or conversations in it — when a White Rabbit with pink eyes runs close by you, and takes a watch out of its waistcoat-pocket, and says, "Oh dear! I shall be too late!"<br><br>You are about to fall a very long way. The trick of it is to wake as yourself.</span>';
  } else {
    let deep;
...
function paintHUD(){
  $('hud').innerHTML=`<div class="hud-panel">${hudSize()}<div class="hud-sep"></div>${hudSelf()}<div class="hud-sep"></div>${hudWonder()}</div>`;
  if(S.mushroom){
...
function render(nodeId, sameNode){
  ...
  (list||[]).forEach(c=>{
    ...
    b.className='choice'+(c.kind?' k-'+c.kind:'')+(c.key?' key':'');
    let pre=c.pre||'';
    if(c.kind && !pre) pre=c.kind;
    b.innerHTML=(pre?`<span class="c-pre">${c.kind?'<b class="kmark">'+c.kind+'</b> · ':''}${pre}</span>`:'')+fmt(c.t);
    b.onclick=()=>choose(c);
    box.appendChild(b);
  });
